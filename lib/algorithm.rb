    ...
    module Neighbors
      ...
      def self.pcr(params)
        ...
        begin
          ...
          start_neighbors_size = 4
          raise "Too few neighbors" if start_neighbors_size > data_matrix.size1
          ...
          for current_neighbors_size in (start_neighbors_size..(data_matrix.size1)).step(step_size)
            ...
            #@r.eval "rmseLoo[dim(rmseLoo)[1],] = c(Inf)" 
          ...
          #if (@r.best_values[1] < (@r.best_values[0]-1))
      ...
      def self.local_svm(acts, sims, type, params)
        ...
        if Algorithm::zero_variance? acts
          ...
          @r = RinRuby.new(false,false) # global R instance leads to Socket errors after a large number of requests
          @r.eval "suppressPackageStartupMessages(library('kernlab'))" # this requires R package "kernlab" to be installed
